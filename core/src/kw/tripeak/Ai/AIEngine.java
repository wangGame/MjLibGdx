...
//    uint8_t m_cbSendCardData;       //发送的牌
    ...
    private int m_cbSendCardData;
//    private int m_MeChairID;
//    private int m_cbLeftCardCount;
//    private int m_cbBankerChair;
    private IPlayer m_MePlayer;
//    private int[][] m_cbCardIndex;
//    private int[][] m_cbDiscardCard;
//    private long[] m_cbDiscardCount;
    GameLogic.TagWeaveItem m_WeaveItemArray[][] = new GameLogic.TagWeaveItem[GAME_PLAYER][MAX_WEAVE];                 //组合
    int m_cbCardIndex[][] = new int[GAME_PLAYER][MAX_INDEX];                         //玩家牌
    int m_cbWeaveItemCount[] = new int[GAME_PLAYER];                               //组合数目
    int m_cbDiscardCount[] = new int[GAME_PLAYER];                                 //丢弃数目
    int m_cbDiscardCard[][] = new int[GAME_PLAYER][MAX_DISCARD];                     //丢弃记录
...
//            scheduleOnce(CC_SCHEDULE_SELECTOR(AIEngine::sendCard), time(NULL) % 2 + 0.8f);
    ...
    public boolean onOutCardEvent(GameCmd.CMD_S_OutCard OutCard) {
        ...
        m_cbDiscardCard[OutCard.cbOutCardUser][(int) m_cbDiscardCount[OutCard.cbOutCardUser]++] = OutCard.cbOutCardData;
        return true;
    }
     ...
     * @return
     */
//    boolean onOperateNotifyEvent(CMD_S_OperateNotify OperateNotify) {
...
//        NLog.i("机器人接收到操作结果事件");
//        GameLogic.TagWeaveItem weaveItem;
